...
class Policy(object):
    ...
    def __init__(self, obs_dim, act_dim, kl_targ, hid1_mult, policy_logvar, base_dir, clipping_range=None):
        """
        Args:
        ...
        """
        self.save_address_check_points = os.path.join(base_dir, 'check_points_policy')
        # self.global_step = 0
    ...
    def save(self):
        ...
        with self.g.as_default():
            ...
            address = os.path.join(self.save_address_check_points, 'policy_net.ckpt')
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            # self.summaries = tf.summary.scalar("p_loss", self.loss)
    ...
    def _policy_nn(self):
        ...
        hid1_size = 64  # self.obs_dim * self.hid1_mult  # 10 empirically determined
        hid3_size = 64  # self.act_dim * 2  # 10 empirically determined
        hid2_size = 64  # int(np.sqrt(hid1_size * hid3_size))
        # heuristic to set learning rate based on NN size (tuned on 'Hopper-v1')
        self.lr = 1e-4  # 9e-4 / np.sqrt(hid2_size)  # 9e-4 empirically determined
        ...
        self.means = 2.0 * tf.sigmoid(out, name="sig_means") - 1.0
...
us step's model
        Calculates log probabilities using previo parameters and
    ...
    def _sample_deterministic(self):
        self.sampled_act_det = self.means
    ...
    def sample(self, obs, flag_det):
        """Draw sample from policy distribution"""
        feed_dict = {self.obs_ph: obs}

        if flag_det:
            return self.sess.run(self.sampled_act_det, feed_dict=feed_dict)
    ...
    def update(self, observes, actions, advantages):
        ...
        for e in range(self.epochs):
            ...
            # summaries,
            loss, kl, entropy = self.sess.run([self.loss, self.kl, self.entropy], feed_dict)  # self.summaries,
