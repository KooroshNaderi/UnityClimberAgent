...
class DenseNetLayer:
    def __init__(self,input:tf.Tensor,initInput:tf.Tensor,nUnits:int,useSkips=True,activation=None):
        ...
        initialW=initialW/np.linalg.norm(initialW)
        #MSRA initialization, used in the original DenseNet paper (https://www.tensorflow.org/api_docs/python/tf/contrib/layers/variance_scaling_initializer)
        ...
        if activation=="relu":
            ...
            raise NameError("Invalid activation type ({}) for DenseNetLayer".format(activation))
        if useSkips:
        ...
        #Same functionality as above, but we init biases such that input distribution's mean maps to zero output.
        #We also init weights to unit norm.
        #This is important as DenseNets are typically very deep and distribution means can shift exponentially further away from
        #zero with increasing depth, leading to NaNs and also very slow convergence, as gradient norm is initially very high,
        #which Adam etc. cope to some extent but take time to adapt to.
        X=initInput
...
class DenseNet:
    def __init__(self,input:tf.Tensor,nLayers:int,nUnitsPerLayer:int, nOutputUnits:int, activation="lrelu", firstLinearLayerUnits:int=0):
        self.layers=[]
        X=input
        initX=input
        ...
        if firstLinearLayerUnits!=0:
            layer=DenseNetLayer(X,initX,firstLinearLayerUnits,useSkips=False,activation=None)
            self.layers.append(layer)
            X,initX=layer.output,layer.initOutput

        #add hidden layers
        for layerIdx in range(nLayers):
            layer=DenseNetLayer(X,initX,nUnitsPerLayer,useSkips=True,activation=activation)
            self.layers.append(layer)
            X,initX=layer.output,layer.initOutput

        #add output layer
        layer=DenseNetLayer(X,initX,nOutputUnits,useSkips=False,activation=None)
        self.layers.append(layer)
...
def denseNet(input:tf.Tensor,nLayers:int,nUnitsPerLayer:int, nOutputUnits:int, activation="selu", firstLinearLayerUnits:int=0):
    instance=DenseNet(input,nLayers,nUnitsPerLayer,nOutputUnits,activation,firstLinearLayerUnits)
...
if __name__ == "__main__":
    ...
    #IMPORTANT: since DenseNets are typically narrow but very deep, they benefit immensely from data-dependent initialization.
    ...
    predictions,initialPredictions=denseNet(input=tfX,nLayers=16,nUnitsPerLayer=8,nOutputUnits=1,activation="lrelu")
